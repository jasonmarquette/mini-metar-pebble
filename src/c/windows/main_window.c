...
static void down_click_handler(
  ...
  APP_LOG(
      APP_LOG_LEVEL_INFO,
      "Next airport requested"
  );

  main_window_set_updated("Next airport...");
  app_message_request_next_airport();
}
...
static void up_click_handler(
  ...
  APP_LOG(
      APP_LOG_LEVEL_INFO,
      "Previous airport requested"
  );

  main_window_set_updated("Previous airport...");
  app_message_request_previous_airport();
}
...
void main_window_set_category(const char *category) {
  ...
  if (strcmp(category, "VFR") == 0) {
    text_layer_set_text_color(s_category_layer, GColorGreen);
  } else if (strcmp(category, "MVFR") == 0) {
    text_layer_set_text_color(s_category_layer, GColorBlue);
  } else if (strcmp(category, "IFR") == 0) {
    text_layer_set_text_color(s_category_layer, GColorRed);
  } else if (strcmp(category, "LIFR") == 0) {
    text_layer_set_text_color(s_category_layer, GColorMagenta);
  } else {
    text_layer_set_text_color(s_category_layer, GColorWhite);
  }
#else
  text_layer_set_text_color(s_category_layer, GColorWhite);
#endif
