...
static void inbox_received_handler(
  ...
  if (category_tuple) {
    weather_set_category(category_tuple->value->cstring);
  }

  if (temperature_tuple) {
    weather_set_temperature_c(temperature_tuple->value->int32);
  }

  if (pressure_tuple) {
    weather_set_pressure_hpa(pressure_tuple->value->int32);
  }

  if (wind_direction_tuple) {
    weather_set_wind_direction(wind_direction_tuple->value->int32);
  }

  if (wind_speed_tuple) {
    weather_set_wind_speed_kt(wind_speed_tuple->value->int32);
  }
...
static void send_request(uint32_t key, const char *description) {
  ...
  if (result != APP_MSG_OK || !iterator) {
    APP_LOG(
        APP_LOG_LEVEL_ERROR,
        "Could not begin outbox for %s: %d",
        description,
        result
    );
    weather_refresh_status();
